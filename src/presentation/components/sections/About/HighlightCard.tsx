...
export interface HighlightCardProps {
  /** Card title */
  title: string;
  /** Card content (supports simple text) */
  content: string;
  /** Icon to display */
...
/**
 * Icon component for highlight cards
 */
function HighlightIcon({ icon }: { icon: AboutMetricIcon }) {
...
/**
 * Card for key highlights (current focus, location, etc.).
 * Single Responsibility: only renders highlight content.
 */
export function HighlightCard({
  ...
  return (
    <BentoCell size="medium" className={cn('group', className)}>
      <motion.div
        ref={ref}
      ...
      >
        <CardWrapper
          className={cn(
            ...
            link && 'cursor-pointer transition-colors hover:bg-white/5'
          )}
          {...wrapperProps}
        >
          {/* Header with icon */}
          ...
          <Typography
            ...
            className={cn(
              'text-white',
              link && 'group-hover:text-primary-400 transition-colors'
            )}
          >
            {content}
          </Typography>
          ...
          {link && (
            <div className="mt-auto pt-3">
              <span className="text-primary-400 inline-flex items-center gap-1 text-sm opacity-0 transition-opacity group-hover:opacity-100">
                Learn more
                <svg
                  className="h-4 w-4"
                  fill="none"
